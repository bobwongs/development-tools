...
func executePythonScript(scriptInBundle: String, sourcePath: String, generationPath: String, source: String?) -> String?
{
    guard let path = Bundle.main.path(forResource: scriptInBundle, ofType: "py") else {
        print("no \(scriptInBundle).py file")
        return nil
    }
    return executeScript(scriptPath: path, sourcePath: sourcePath, generationPath: generationPath, source: source, launchPath: "/usr/bin/python", arguments: [path])
}
